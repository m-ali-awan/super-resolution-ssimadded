...
class Loss(nn.Module):
    ...
    def forward(self, generator, discriminator, hr_real, lr_real):
        ...
        #my line
        hr_fake=F.interpolate(hr_fake,size=hr_real[0][0].shape)
        #--------
        ...
        g_loss = (
            0.9 * self.adv_loss(fake_preds_for_g, False) + \
            0.6 * self.vgg_loss(hr_real, hr_fake) + \
            self.img_loss(hr_real, hr_fake)
        )
        d_loss = 0.5 * (
            0.2*self.adv_loss(real_preds_for_d, True) + \
            2*self.adv_loss(fake_preds_for_d, False)
        )
